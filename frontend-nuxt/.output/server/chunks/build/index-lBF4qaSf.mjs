import { b as useAuth, h as useToast, c as __nuxt_component_4, f as __nuxt_component_1, _ as _export_sfc, d as useUI, m as mergeConfig, e as appConfig } from './server.mjs';
import { _ as __nuxt_component_2 } from './Card-Da4Q_95c.mjs';
import { defineComponent, ref, computed, mergeProps, withCtx, createTextVNode, createVNode, toDisplayString, openBlock, createBlock, withModifiers, createCommentVNode, Fragment, renderList, resolveComponent, renderSlot, toRef, useId, useSSRContext } from 'vue';
import { h as he, S as Se, G as Ge, Y as Ye, s as s$4 } from './transition-BMgUVLZM.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const mainTabs = [
      ...
      { label: "Pessoa Jur\xEDdica (PJ)", value: "business_pj" },
      { label: "Pessoa F\xEDsica (PF)", value: "personal_pf" },
      { label: "Neutro / Transfer\xEAncias", value: "transfer" }
    ];
    const sourceTabs = [
      { label: "Conta Corrente", value: "checking_account", icon: "i-heroicons-wallet" },
      { label: "Cart\xE3o de Cr\xE9dito", value: "credit_card", icon: "i-heroicons-credit-card" }
    ];
    ...
    const gastosLabel = computed(() => {
      switch (activeTab.value) {
        ...
        case "personal_pf":
          return "Sa\xEDdas (PF)";
        case "transfer":
          return "Sa\xEDdas (Neutras)";
        default:
          return "Sa\xEDdas (Pendentes)";
      }
    });
    const lucroLabel = computed(() => {
      switch (activeTab.value) {
        case "business_pj":
          return "Lucro L\xEDquido";
        case "personal_pf":
          return "Saldo (PF)";
        case "transfer":
          return "Saldo (Neutro)";
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (response && response.success) {
          ...
          toast.add({
            title: "Erro ao carregar hist\xF3rico",
            description: (response == null ? void 0 : response.message) || "Falha ao buscar dados",
            color: "red"
          });
        }
      } catch (err) {
      ...
      try {
        ...
        if (response && response.success) {
          ...
          toast.add({
            ...
            description: "Extrato processado com sucesso. Classifique os lan\xE7amentos.",
            color: "green"
          });
        } else {
          toast.add({
            title: "Falha no upload",
            description: (response == null ? void 0 : response.message) || "Erro ao processar o arquivo.",
            color: "red"
          });
        }
      } catch (err) {
    ...
    const handleConfirmImport = async () => {
      ...
      try {
        ...
        if (response && response.success) {
          toast.add({
            title: "Lan\xE7amentos salvos!",
            description: "Transa\xE7\xF5es salvas e concilia\xE7\xE3o realizada com sucesso.",
            color: "green"
          });
          tempTransactions.value = [];
          file.value = null;
          ...
          toast.add({
            ...
            description: (response == null ? void 0 : response.message) || "Falha ao salvar lan\xE7amentos.",
            color: "red"
          });
        }
      } catch (err) {
        toast.add({
          title: "Erro de conex\xE3o",
          description: err.message || "Falha ao salvar lan\xE7amentos.",
          color: "red"
        });
      } finally {
    ...
    const handleReclassifySaved = async (id, classification) => {
      try {
        ...
        if (response && response.success) {
          ...
          toast.add({
            title: "Classifica\xE7\xE3o atualizada",
            description: "Classifica\xE7\xE3o atualizada com sucesso.",
            color: "green"
          });
        } else {
          toast.add({
            title: "Erro ao reclassificar",
            description: (response == null ? void 0 : response.message) || "Erro ao alterar classifica\xE7\xE3o.",
            color: "red"
          });
        }
      } catch (err) {
        toast.add({
          title: "Erro de conex\xE3o",
          description: err.message || "Erro ao alterar classifica\xE7\xE3o.",
          color: "red"
        });
      }
    };
    ...
    const confirmDelete = async () => {
      ...
      try {
        ...
        if (response && response.success) {
          ...
          toast.add({
            title: "Lan\xE7amento exclu\xEDdo",
            description: "Transa\xE7\xE3o exclu\xEDda com sucesso.",
            color: "green"
          });
        } else {
          toast.add({
            title: "Erro ao excluir",
            description: (response == null ? void 0 : response.message) || "Erro ao excluir transa\xE7\xE3o.",
            color: "red"
          });
        }
      } catch (err) {
        toast.add({
          title: "Erro de conex\xE3o",
          description: err.message || "Erro de conex\xE3o ao excluir transa\xE7\xE3o.",
          color: "red"
        });
      }
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (fetchLoading.value) {
        ...
        _push(`<div class="p-12 text-center text-sm text-gray-500"> Nenhuma transa\xE7\xE3o encontrada no banco de dados para os filtros selecionados. </div>`);
      } else {
        _push(`<div class="overflow-x-auto w-full"><table class="w-full text-sm text-left"><thead class="bg-gray-50 dark:bg-zinc-900 text-xs font-semibold uppercase text-gray-500 dark:text-zinc-400 border-b border-gray-200 dark:border-zinc-800"><tr><th class="px-4 py-3">Data</th><th class="px-4 py-3">Descri\xE7\xE3o</th><th class="px-4 py-3">Valor</th><th class="px-4 py-3 text-center">Classifica\xE7\xE3o</th><th class="px-4 py-3 text-center">A\xE7\xF5es</th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-zinc-800"><!--[-->`);
      ...
      _push(ssrRenderComponent(_component_UModal, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="flex flex-col max-h-[90vh] overflow-hidden bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg shadow-xl"${_scopeId}><div class="p-6 pb-4 border-b border-gray-200 dark:border-zinc-800 shrink-0"${_scopeId}><h3 class="text-lg font-bold text-gray-900 dark:text-white"${_scopeId}>Concilia\xE7\xE3o de Extrato OFX</h3><p class="text-xs text-gray-500 mt-1"${_scopeId}> Origem selecionada: <span class="font-semibold"${_scopeId}>${ssrInterpolate(activeSource.value === "checking_account" ? "Conta Corrente" : "Cart\xE3o de Cr\xE9dito")}</span></p></div><div class="p-6 overflow-y-auto space-y-4 flex-1"${_scopeId}>`);
            if (tempTransactions.value.length === 0) {
              ...
              _push2(`<div class="space-y-4"${_scopeId}><div class="border border-gray-200 dark:border-zinc-800 rounded-md overflow-hidden bg-white dark:bg-zinc-900"${_scopeId}><div class="overflow-x-auto"${_scopeId}><table class="w-full text-sm text-left"${_scopeId}><thead class="bg-gray-50 dark:bg-zinc-950 text-xs font-semibold uppercase text-gray-500"${_scopeId}><tr${_scopeId}><th class="px-4 py-3"${_scopeId}>Data</th><th class="px-4 py-3"${_scopeId}>Descri\xE7\xE3o</th><th class="px-4 py-3"${_scopeId}>Valor</th><th class="px-4 py-3 text-center"${_scopeId}>Classifica\xE7\xE3o</th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-zinc-800"${_scopeId}><!--[-->`);
            ...
            return [
              createVNode("div", { class: "flex flex-col max-h-[90vh] overflow-hidden bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg shadow-xl" }, [
                createVNode("div", { class: "p-6 pb-4 border-b border-gray-200 dark:border-zinc-800 shrink-0" }, [
                  createVNode("h3", { class: "text-lg font-bold text-gray-900 dark:text-white" }, "Concilia\xE7\xE3o de Extrato OFX"),
                  createVNode("p", { class: "text-xs text-gray-500 mt-1" }, [
                    createTextVNode(" Origem selecionada: "),
                    createVNode("span", { class: "font-semibold" }, toDisplayString(activeSource.value === "checking_account" ? "Conta Corrente" : "Cart\xE3o de Cr\xE9dito"), 1)
                  ])
                ]),
                        ...
                        createVNode("table", { class: "w-full text-sm text-left" }, [
                          createVNode("thead", { class: "bg-gray-50 dark:bg-zinc-950 text-xs font-semibold uppercase text-gray-500" }, [
                            createVNode("tr", null, [
                              ...
                              createVNode("th", { class: "px-4 py-3" }, "Descri\xE7\xE3o"),
                              createVNode("th", { class: "px-4 py-3" }, "Valor"),
                              createVNode("th", { class: "px-4 py-3 text-center" }, "Classifica\xE7\xE3o")
                            ])
                          ]),
      ...
      _push(ssrRenderComponent(_component_UModal, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<h3 class="text-lg font-bold"${_scopeId}>Excluir Lan\xE7amento</h3></div><div class="pt-2 text-sm text-gray-500 dark:text-zinc-400 text-left"${_scopeId}> Voc\xEA est\xE1 prestes a excluir esta transa\xE7\xE3o permanentemente. Esta a\xE7\xE3o atualizar\xE1 seus relat\xF3rios financeiros. </div><p class="text-[11px] text-gray-400 dark:text-zinc-500 pt-1.5 font-semibold text-left"${_scopeId}> Esta a\xE7\xE3o \xE9 irrevers\xEDvel e remover\xE1 permanentemente os registros do banco de dados. </p><div class="pt-4 flex justify-end gap-3"${_scopeId}>`);
            _push2(ssrRenderComponent(_component_UButton, {
              color: "gray",
              variant: "ghost",
            ...
            _push2(ssrRenderComponent(_component_UButton, {
              ...
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(` Confirmar Exclus\xE3o `);
                } else {
                  return [
                    createTextVNode(" Confirmar Exclus\xE3o ")
                  ];
                }
              }),
              _: 1
            ...
            return [
              createVNode("div", { class: "p-6 bg-white dark:bg-zinc-900 rounded-lg shadow-xl border border-gray-200 dark:border-zinc-800" }, [
                createVNode("div", { class: "flex items-center gap-2 text-red-600 dark:text-red-500 mb-1" }, [
                  ...
                  createVNode("h3", { class: "text-lg font-bold" }, "Excluir Lan\xE7amento")
                ]),
                createVNode("div", { class: "pt-2 text-sm text-gray-500 dark:text-zinc-400 text-left" }, " Voc\xEA est\xE1 prestes a excluir esta transa\xE7\xE3o permanentemente. Esta a\xE7\xE3o atualizar\xE1 seus relat\xF3rios financeiros. "),
                createVNode("p", { class: "text-[11px] text-gray-400 dark:text-zinc-500 pt-1.5 font-semibold text-left" }, " Esta a\xE7\xE3o \xE9 irrevers\xEDvel e remover\xE1 permanentemente os registros do banco de dados. "),
...
//# sourceMappingURL=index-lBF4qaSf.mjs.map
